...
class TokenManager {
  ...
  loadTokensFromEnv(type) {
    try {
      if (type === 'bot') {
        ...
        return {
          accessToken: accessToken.startsWith('oauth:')
            ? accessToken
            : `oauth:${accessToken}`,
  ...
  async initializeAuthProviders() {
    try {
      ...
      if (
        !this.tokenData.bot?.accessToken ||
        !this.tokenData.bot?.refreshToken
      ) {
      ...
      if (
        !this.tokenData.broadcaster?.accessToken ||
        !this.tokenData.broadcaster?.refreshToken
      ) {
        throw new Error(
          'Broadcaster tokens not found in environment variables'
        );
      }
  ...
  async validateTokens() {
    try {
      ...
      if (
        this.tokenData.bot &&
        now - this.tokenData.bot.obtainmentTimestamp >
          this.tokenData.bot.expiresIn * 1000
      ) {
        needsRefresh = true;
      }
