...
export async function getModStats() {
  try {
    const stats = {
      ...
      suspiciousRaids: moderationData.raidHistory.filter((r) => r.suspicious).length,
    };
...
export async function getUserHistory(targetUser) {
  try {
    const userStats = moderationData.chatStats.get(targetUser.toLowerCase()) || {
      messages: 0,
      timeouts: 0,
      bans: 0,
      lastActive: null,
    ...
    const warnings = moderationData.warnings.get(targetUser.toLowerCase()) || [];
...
export async function untrustUser(targetUser) {
  try {
    const removed = moderationData.trustedUsers.delete(targetUser.toLowerCase());
...
export async function getRaidHistory() {
  try {
    const recentRaids = moderationData.raidHistory
      .slice(-5)
      .map((raid) => `${raid.username} (${raid.viewers} viewers)${raid.suspicious ? ' ⚠️' : ''}`)
      .join(' | ');
...
export function moderateMessage(message, username, userLevel) {
  try {
    ...
    const suspicious = {
      excessiveCaps: message.replace(/[^A-Z]/g, '').length > message.length * 0.7,
