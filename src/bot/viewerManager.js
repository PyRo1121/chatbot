...
class ViewerManager {
  ...
  async handleRaid(raider, viewers) {
    try {
      ...
      const raidHistory = this.data.raids.filter(
        (raid) => raid.raider === raider
      );
...
Keep it fun and personal! Message should be one short paragraph.`;
  ...
  calculateReturningRate() {
    const returningViewers = Object.values(this.data.viewers).filter(
      (v) => v.visits > 1
    ).length;
  ...
  checkLoyaltyMilestone(username) {
    ...
    const existingMilestone = viewer.milestones.find(
      (m) => m.level === currentLevel.title
    );
  ...
  async generateShoutout(username) {
    try {
      ...
      // Get channel info
      const channel = await client.twitchApi.channels.getChannelInfoById(
        user.id
      );
