...
class StreamAnalytics {
  ...
  calculateHealthScore(stream) {
    ...
    if (stream.viewers && stream.viewers.length > 1) {
      const retention =
        stream.viewers[stream.viewers.length - 1] / Math.max(...stream.viewers);
  ...
  calculateEngagement() {
    ...
    const engagements = streams.map((stream) => {
      ...
      const commandsPerHour =
        Object.values(stream.commands || {}).reduce((a, b) => a + b, 0) /
        (duration / 60);
      return (chatPerMinute * 0.7 + commandsPerHour * 0.3) / 2;
    });

    return Math.round(
      (engagements.reduce((a, b) => a + b, 0) / engagements.length) * 100
    );
  }
  ...
  getBestCategory() {
    const categories = Object.entries(
      this.analyticsData.performance.categories
    );
  ...
  getPeakConcurrent() {
    ...
    return Math.max(
      ...streams.map((stream) => Math.max(...(stream.viewers || [0])))
    );
  }
    ...
    try {
      ...
      Object.values(this.analyticsData.streams).forEach((stream) => {
        if (stream.category) {
          ...
          category.totalViewers +=
            stream.viewers?.reduce((a, b) => a + b, 0) || 0;
          category.streams++;
