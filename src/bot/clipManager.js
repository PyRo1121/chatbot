...
class ClipManager {
  ...
  async addClip(clip) {
    try {
      ...
      // Update categories
      if (clip.game) {
        this.clipsData.categories[clip.game] =
          (this.clipsData.categories[clip.game] || 0) + 1;
      }
  ...
  async getClipsByTag(tag) {
    ...
    return this.clipsData.clips.filter((clip) =>
      this.extractTags(clip.title).then((tags) => tags.includes(normalizedTag))
    );
  }
  ...
  async getRecentClips(days = 7) {
    ...
    return this.clipsData.clips.filter(
      (clip) => new Date(clip.timestamp) > cutoff
    );
  }

  async getTopClips(limit = 10) {
    return this.clipsData.clips
  ...
  async getHighlights(days = 7) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - days);
    return this.clipsData.highlights.filter(
      (highlight) => new Date(highlight.timestamp) > cutoff
    );
  }
  ...
  cleanup() {
    try {
      ...
      this.clipsData.clips.forEach((clip) => {
        if (clip.game) {
          this.clipsData.categories[clip.game] =
            (this.clipsData.categories[clip.game] || 0) + 1;
        }
        this.extractTags(clip.title).then((tags) => {
          tags.forEach((tag) => {
            this.clipsData.tags[tag] = (this.clipsData.tags[tag] || 0) + 1;
          });
        });
