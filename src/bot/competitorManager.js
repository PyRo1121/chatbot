...
export async function trackChannel(twitchClient, channelName) {
  try {
    ...
    // Get channel info
    const channelInfo =
      await twitchClient.twitchApi.users.getUserByName(channelName);
    if (!channelInfo) {
  ...
  try {
    ...
    for (const [channelName, data] of competitorData.trackedChannels) {
      ...
      if (stream) {
        ...
        // Update categories
        const categoryIndex = data.stats.topCategories.findIndex(
          (c) => c.name === stream.gameName
        );
        if (categoryIndex >= 0) {
...
export async function getSuggestions(twitchClient) {
  try {
    ...
    const successfulTitles = Array.from(competitorData.trackedChannels.values())
      .filter(
        (data) => data.stats.lastStream?.viewers > data.stats.averageViewers
      )
