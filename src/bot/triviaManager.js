...
function endQuestion(channel) {
  ...
  correctAnswers.forEach((answer, index) => {
    const currentPoints = triviaState.participants.get(answer.username)?.points || 0;
    const points =
      index === 0
        ? triviaState.settings.pointsPerQuestion + triviaState.settings.bonusPoints
...
export async function endTrivia(channel) {
  try {
    ...
    const standings = Array.from(triviaState.participants.entries())
      ...
      .slice(0, 3)
      .map((entry, index) => `${index + 1}. ${entry[0]} (${entry[1].points} pts)`);
