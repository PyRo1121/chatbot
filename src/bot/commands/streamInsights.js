...
export async function handleTitle(client, channel, user, args) {
  try {
    ...
    await client.twitchApi.channels.updateChannelInfo(process.env.TWITCH_CHANNEL_ID, {
      title: newTitle,
    });
...
export async function handleUptime(client, channel, user) {
  try {
    const stream = await client.twitchApi.streams.getStreamByUserId(process.env.TWITCH_CHANNEL_ID);

    if (!stream) {
      return 'Stream is currently offline!';
    }

    const uptime = Math.floor((Date.now() - stream.startDate.getTime()) / 1000 / 60);
...
export async function handleMilestone(client, channel, user, args) {
  try {
    ...
    const message = announcement || `We just hit a milestone: ${description}! 🎉`;
