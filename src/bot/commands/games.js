...
class ChatGames {
  ...
  trackActivity(username, type) {
    ...
    if (
      ...
      Array.from(this.activeGames.values()).some((game) => game.participants.has(username))
    ) {
      stats.gamesPlayed++;
    }
  }
  ...
  checkTrivia(username, answer) {
    ...
    if (answer.toLowerCase().includes(this.trivia.answer)) {
      const timeTaken = ((Date.now() - this.trivia.startTime) / 1000).toFixed(1);
      const result = {
        success: true,
  ...
  checkWordChain(username, word) {
    ...
    if (Date.now() - this.wordChain.startTime > 300000 || this.wordChain.usedWords.size >= 30) {
      const result = {
        success: true,
  ...
  createWordScramble() {
    const words = ['streamer', 'broadcast', 'community', 'moderator', 'emote', 'subscriber'];
