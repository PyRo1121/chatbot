...
export function handleUserCommands() {
  const userCommands = commandList.filter((cmd) => {
    const lowerCmd = cmd.toLowerCase();
    return (
      !lowerCmd.includes('(mods only)') &&
      !lowerCmd.includes('(broadcaster only)')
    );
  });

  return {
    success: true,
    message: 'Available commands: ' + userCommands.join(' | '),
  };
}

export function handleModCommands(userLevel) {
  if (userLevel !== 'mod' && userLevel !== 'broadcaster') {
    return {
      success: false,
      message: 'This command is for moderators and broadcasters only!',
    };
  }

  const modCommands = commandList.filter((cmd) => {
    const lowerCmd = cmd.toLowerCase();
    return (
      lowerCmd.includes('(mods only)') ||
      lowerCmd.includes('(broadcaster only)')
