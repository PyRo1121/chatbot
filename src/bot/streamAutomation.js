...
Make it exciting and inviting. Keep it under 200 characters.`;
      const message = await generateResponse(prompt);
      return message || `Stream is starting! Playing ${data.category}: ${data.title}`;
    });
  }
  ...
  async checkStreamStatus() {
    ...
    try {
      ...
      const stream = await this.client.apiClient.streams.getStreamByUserId(this.client.channelId);
  ...
  async handleStreamStart(stream) {
    try {
      ...
      const message = await this.automationState.customActions.get('streamStart')({
  ...
  async rotateScheduledMessages() {
    if (!this.automationState.isLive || this.automationState.scheduledMessages.length === 0) {
      return;
    }
    try {
  ...
  async handleMilestone(description) {
    try {
      const message = await this.automationState.customActions.get('milestone')({ description });
      await this.client.client.say(process.env.TWITCH_CHANNEL, message);
  ...
  async handleRaid(raider, viewers) {
    try {
      const message = await this.automationState.customActions.get('raidWelcome')({
        raider,
        viewers,
      });
      await this.client.client.say(process.env.TWITCH_CHANNEL, message);
