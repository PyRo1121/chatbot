  ...
  removeSpamPattern(name) {
    ...
    if (removed) {
      this.moderationData.spamPatterns =
        this.moderationData.spamPatterns.filter((p) => p.name !== name);
      this.saveModerationData();
    }
    return removed;
  }
...
export const assessRaid =
  advancedModeration.assessRaid.bind(advancedModeration);
