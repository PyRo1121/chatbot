...
class FollowProtection {
  ...
  async checkFollower(username, accountAge, followCount) {
    try {
      ...
      if (existingSuspicious) {
        reasons.push(`Previously marked as suspicious: ${existingSuspicious.reason}`);
      }
  ...
  cleanup() {
    try {
      ...
      this.protectionData.suspiciousFollowers = this.protectionData.suspiciousFollowers.filter(
