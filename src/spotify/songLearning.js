...
class SongLearning {
  ...
  loadLearningData() {
    try {
      const data = readFileSync(join(process.cwd(), 'src/spotify/song_learning.json'), 'utf8');
  ...
  initializePatterns() {
    ...
    this.learningData.karaokePatterns = Array.from(
      new Set([...defaultKaraokePatterns, ...(this.learningData.karaokePatterns || [])])
    );

    this.learningData.trollPatterns = Array.from(
      new Set([...defaultTrollPatterns, ...(this.learningData.trollPatterns || [])])
    );
  ...
  async learnFromRequest(query, username, trackInfo = null) {
    try {
      ...
      this.learningData.songPatterns[songKey] = (this.learningData.songPatterns[songKey] || 0) + 1;
      ...
      if (trackInfo) {
        ...
        const songParts = normalizedQuery.split(' - ').map((part) => part.trim());
  ...
  isTrollSong(query) {
    return this.learningData.trollPatterns.some((pattern) => query.toLowerCase().includes(pattern));
  }
