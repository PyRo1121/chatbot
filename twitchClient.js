...
class TwitchClient {
  ...
  async generateResponse(prompt) {
    try {
      const completion = await this.openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [
          { role: "system", content: "You are a funny Twitch chat bot. Keep responses short and entertaining." },
  ...
  async onMessageHandler(channel, tags, message, self) {
    ...
    if (tags['first-msg']) {
      const response = await this.generateResponse(`Generate a funny welcome message for a first-time chatter`);
    ...
    if (message.startsWith('!hello')) {
      const response = await this.generateResponse(`Generate a funny response to someone saying hello`);
  ...
  async handleStatsCommand(channel) {
    try {
      ...
      this.client.say(channel, 'Error fetching stream stats. Make sure your Twitch API credentials are correct');
    }
  }
    ...
    try {
      ...
      if (stream) {
        ...
        this.streamAnalytics.averageViewers = Math.round(
          (this.streamAnalytics.averageViewers * this.streamAnalytics.totalStreams + stream.viewers) /
          (this.streamAnalytics.totalStreams + 1)
        );
        this.streamAnalytics.peakViewers = Math.max(this.streamAnalytics.peakViewers, stream.viewers);
  ...
  async handleBestTimesCommand(channel) {
    try {
      ...
      const bestTimes = sortedTimes.map(([hour, count]) => {
        const time = new Date();
        time.setHours(hour);
  ...
  async onSubscription(channel, username, method, message, userstate) {
    const response = await this.generateResponse(`Generate a funny thank you message for a new Twitch subscriber`);
    this.client.say(channel, `@${username}, ${response}`);
  }

  async onResub(channel, username, months, message, userstate, methods) {
    const response = await this.generateResponse(`Generate a funny thank you message for a Twitch resubscription`);
    this.client.say(channel, `@${username}, ${response}`);
  }

  async onRaid(channel, username, viewers) {
    const response = await this.generateResponse(`Generate a funny welcome message for a Twitch raid`);
    this.client.say(channel, `@${username}, ${response}`);
  }

  async onFollow(channel, username, userstate) {
    const response = await this.generateResponse(`Generate a funny thank you message for a new Twitch follower`);
    this.client.say(channel, `@${username}, ${response}`);
  }
